...
async function bootstrap() {
  ...
  try {
    ...
    worksheet.eachRow((row, rowNumber) => {
      if (rowNumber > 1 && rowNumber <= 4) { // rows 2,3,4 (first 3 data rows)
        const rowData: any = {};
    ...
    const mappedLines: D365FOCustomerPaymentJournalLineRequest[] = rows.map((row, index) => {
      return {
        dataAreaId: 'm-p',
        LineNumber: index + 1,
        cashDirection: 'out',
        customLineApiBody: {
          ...
          MARKEDINVOICE: '',
          offsetAccountDisplayValue: String(row['OFFSETACCOUNTDISPLAYVALUE'] || ''),
          OffsetAccountTypeStr: String(row['OFFSETACCOUNTTYPE'] || '') as any,
          OffsetCompany: 'm-p',
          OFFSETFINTAGDISPLAYVALUE: String(row['OFFSETFINTAGDISPLAYVALUE'] || ''),
    ...
    const result = await customerPaymentJournalService.postCashOutLinesForHeader(
      headerKey,
      mappedLines,
      1,
      'm-p'
