...
class TestProcessor extends BaseCashEntryProcessor {
  ...
  protected isInbound() { return false; }
  protected isTrucking() { return false; }
...
const rawData = JSON.parse(fs.readFileSync(__dirname + '/test_data.json', 'utf8'));
...
const withholdingBefore = models.filter(m => m.ACCOUNTTYPE === 'Ledger' && m.ACCOUNTDISPLAYVALUE.startsWith('223304')).length;
console.log(`\nBefore processing: Found ${withholdingBefore} withholding lines.`);

const processor = new TestProcessor();
const result = processor.test(models);

const withholdingAfter = result.lines.filter(m => m.ACCOUNTTYPE === 'Ledger' && m.ACCOUNTDISPLAYVALUE.startsWith('223304')).length;
console.log(`After processing: Found ${withholdingAfter} withholding lines remaining.\n`);
