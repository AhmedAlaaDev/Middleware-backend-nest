...
async function validateParity() {
  ...
  const istConvertedPath = 'e:\\OneDrive - MESCO\\Desktop\\MiddleWare\\D365FOMiddleware_Nestbackend\\Dynamics_Converted_To_IST.xlsx';
  ...
  console.log(`   - Status: ${dynRows === istRows ? 'PASSED (0 rows lost) ✅' : 'FAILED ❌'}`);
  ...
  dynSheet.eachRow((row, rowNumber) => {
    ...
    const tx = Number(row.getCell(dynHeaders['Amount in transaction currency'] || 10).value) || 0;
  ...
  console.log(`   - Difference: ${Math.abs(dynTotalTxAmount - istNetTxAmount).toFixed(4)}`);
  console.log(`   - Status: ${Math.abs(dynTotalTxAmount - istNetTxAmount) < 0.01 ? 'PASSED (100% Exact Financial Parity) ✅' : 'FAILED ❌'}`);
  ...
  console.log(`   - Difference: ${Math.abs(dynTotalEgpAmount - istNetEqvAmount).toFixed(4)}`);
  console.log(`   - Status: ${Math.abs(dynTotalEgpAmount - istNetEqvAmount) < 0.01 ? 'PASSED (100% Exact EGP Parity) ✅' : 'FAILED ❌'}`);
  ...
  dynSheet.getRow(100).eachCell((cell, colNumber) => {
    dynRow100[dynSheet.getRow(1).getCell(colNumber).value as string] = cell.value;
  });

  const istRow100: Record<string, any> = {};
  istSheet.getRow(100).eachCell((cell, colNumber) => {
    istRow100[istSheet.getRow(1).getCell(colNumber).value as string] = cell.value;
  });

  console.log(`   - Dynamics Voucher: ${dynRow100['Voucher']} -> Converted VOUCHER: ${istRow100['VOUCHER']}`);
  console.log(`   - Dynamics Journal: ${dynRow100['Journal number']} -> Converted JOURNALNAME: ${istRow100['JOURNALNAME']}`);
  console.log(`   - Dynamics Tx Amount: ${dynRow100['Amount in transaction currency']} -> DEBIT: ${istRow100['DEBIT']}, CREDIT: ${istRow100['CREDIT']}`);
  console.log(`   - Dynamics Payment reference: ${dynRow100['Payment reference']} -> PAYMENTREFERENCE: ${istRow100['PAYMENTREFERENCE']}`);
  console.log(`   - Dynamics Document2/Document: ${dynRow100['Document2'] || dynRow100['Document']} -> DOCUMENT: ${istRow100['DOCUMENT']}`);
}
