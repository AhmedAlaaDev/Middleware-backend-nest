...
const models = mockData.map(d => {
  ...
  model.ACCOUNTTYPE = d.ACCOUNTTYPE as any;
...
console.log('--- BEFORE ---');
models.forEach(m => console.log(`${m.ACCOUNTTYPE} | ${m.VOUCHER} | ${m.INVOICE} | Debit: ${m.DEBITAMOUNT} | Credit: ${m.CREDITAMOUNT}`));

const processor = new TestProcessor();
const result = processor.test(models);

console.log('\n--- AFTER ---');
result.lines.forEach(m => console.log(`${m.ACCOUNTTYPE} | ${m.VOUCHER} | ${m.INVOICE} | Debit: ${m.DEBITAMOUNT} | Credit: ${m.CREDITAMOUNT}`));
