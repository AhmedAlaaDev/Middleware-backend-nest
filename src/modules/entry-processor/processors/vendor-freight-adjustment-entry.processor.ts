...
export class VendorFreightAdjustmentEntryProcessor extends EntryProcessorBase {
  ...
  public async formatAndEnrichAsync(
    ...
    const grouped = this.groupByUniqueId(
      data.map((d) => new VendorFreightAdjustmentRawData(d)),
    );
    ...
    for (const [uniqueId, lines] of grouped.entries()) {
      ...
      const header = new IVendorFreightAdjustmentDFOHeader({
        journalBatchNum,
  ...
  private groupByUniqueId(lines: VendorFreightAdjustmentRawData[]) {
    const sorted = [...lines].sort((a, b) => a.UniqueId - b.UniqueId);
    const grouped = new Map<string, VendorFreightAdjustmentRawData[]>();
