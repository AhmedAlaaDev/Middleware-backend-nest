...
@Injectable()
export class ClosingFreightDifferenceEntryProcessor extends EntryProcessorBase {
  private readonly procLogger = new Logger(
    ClosingFreightDifferenceEntryProcessor.name,
  );
  ...
  private async loadCounters(_company: string): Promise<{
    ...
    const voucherSetting = await this.queryBus.execute(
      new GetSettingQuery(
        'last.ledger.closing.freight.difference.voucher.number',
      ),
    );
