...
import { ProcessClosingFreightDifferenceCommand } from '../process-closing-freight-difference.command';
...
@CommandHandler(ProcessClosingFreightDifferenceCommand)
@Injectable()
export class ProcessClosingFreightDifferenceHandler implements ICommandHandler<ProcessClosingFreightDifferenceCommand> {
  private readonly logger = new Logger(
    ProcessClosingFreightDifferenceHandler.name,
  );
  ...
  public async execute(
    command: ProcessClosingFreightDifferenceCommand,
  ): Promise<IDataBatch> {
    this.logger.log(
      `Start ProcessClosingFreightDifference: company=${command.companyId}, bufferLength=${command.fileBuffer?.length || 0}`,
    );
    ...
    >(
      EntryProcessorTypes.LedgerClosingFreightDifference,
      ENTRY_PROCESSOR_NAMES.LEDGER_CLOSING_FREIGHT_DIFFERENCE,
      command.companyId,
      `Closing Freight Difference ${Date.now()}`,
      rawData,
