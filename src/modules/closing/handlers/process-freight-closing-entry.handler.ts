...
import { ProcessFreightClosingEntryCommand } from '@/modules/closing/commands';
import {
  ClosingEntryModel,
...
export class ProcessFreightClosingEntryHandler implements ICommandHandler<ProcessFreightClosingEntryCommand> {
  ...
  public async execute(
    command: ProcessFreightClosingEntryCommand,
  ): Promise<IDataBatch> {
    this.logger.log(
      `Start ProcessFreightClosingEntry: company=${command.companyId}, bufferLength=${command.fileBuffer?.length || 0}`,
    );
    ...
    >(
      EntryProcessorTypes.LedgerFreightClosingEntry,
      ENTRY_PROCESSOR_NAMES.LEDGER_FREIGHT_CLOSING_ENTRY,
      command.companyId,
      `Freight Closing Entry ${Date.now()}`,
      rawData,
