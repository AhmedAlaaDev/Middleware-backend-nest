...
import { UpdateFinancialDimensionsCommand } from '../update-financial-dimensions.command';

@CommandHandler(UpdateFinancialDimensionsCommand)
export class UpdateFinancialDimensionsHandler
  implements ICommandHandler<UpdateFinancialDimensionsCommand>
{
  private readonly logger = new Logger(UpdateFinancialDimensionsHandler.name);
  ...
  public async execute(
    command: UpdateFinancialDimensionsCommand,
  ): Promise<{ dimensionsUpdated: number; dimensionValuesUpdated: number }> {
    this.logger.log(
      `Updating financial dimensions from D365FO${command.company ? ` for company: ${command.company}` : ''}`,
    );
    ...
    while (hasMore) {
      const response = await this.dimensionService.getDimensionList({
        useCache: false, // Don't use cache for update operations
        maxCount: pageSize,
        skipCount: skipCount,
      });
    ...
    for (const dim of allDimensions) {
      ...
      // Update or insert dimension values
      for (const dv of allDimensionValues) {
        ...
        if (existingValue) {
          // Update existing value if description changed
    ...
    this.logger.log(
      `Update completed: ${dimensionsUpdated} dimensions updated, ${dimensionValuesUpdated} dimension values updated`,
    );
